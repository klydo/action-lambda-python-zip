...
INPUT_LAMBDA_CONFIGS_PATH="aws_lambda/functions/"
INPUT_LAMBDA_HANDLER="aws_lambda.main.lambda_handler"

INPUT_LAMBDA_FUNCTION_PREFIX="scopingAPI"
INPUT_LAMBDA_FUNCTION_SUFFIX=""
...
create_or_update_function_code(){
    ...
    then
        ...
        aws lambda create-function --function-name $LAMBDA_FUNCTION_NAME --runtime $settings_runtime --role $settings_role --handler $INPUT_LAMBDA_HANDLER --zip-file fileb://code.zip
    else
...
update_lambda_configuration() {
    aws lambda update-function-configuration --function-name $entrypoint --layers ${ALL_LAMBDA_LAYERS} --runtime $settings_runtime --role $settings_role --handler $INPUT_LAMBDA_HANDLER --timeout $settings_timeout --memory-size $settings_memory
}
